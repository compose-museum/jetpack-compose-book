![实战Compose——做个简洁却不简单的星球打卡App](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/221a137e94094f78ac108199872a9aa1~tplv-k3u1fbpfcp-zoom-crop-mark:1304:1304:1304:734.awebp?)

# Focus

> Focus是一款帮助你集中的app——为自己的目标建立星球，将时间投入在上面。花在星球上的每一分钟都会被记录，每颗星球可以定制颜色与外观。为了贯彻简单干净不让人分心的设计理念，app采用白灰为主色调。为了不使界面显得单调，所以增加了不少的动画。如果您觉得还可以，希望🙏您能点个Star!!!! [ComposeFocus]([github.com/Watermelon0…](https://github.com/Watermelon02/ComposeFocus))
...
Compose的`声明式`写法和一些`手势api`让许多控件实现起来更为简单。 比如项目主界面中的星球倒计时时钟，这个时钟既需要能够处理用户的手指滑动来设置倒计时时间，还需要能够在用户长按之后开始倒计时。
...
compose中自带不少强大的、可扩展的动画 API，可以轻松的实现一些效果。比如———— `AnimatedVisibility()`配合`ModalBottomSheetLayout()`实现伸缩列表：
...
因为时间限制所以还有很多想要实现的功能没来得及做，不出意外的话之后会继续修改bug和增加功能。最后再次希望您能够给个点个Star!!!! [ComposeFocus]([github.com/Watermelon0…](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2FWatermelon02%2FComposeFocus))
