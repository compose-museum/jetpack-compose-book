...
## 前言
了解过 `Composeble` 的同学都知道，只需要添加一个 `@Composeble` 注解就可以将函数转化成 `Composeble` 函数，同时 `Composeble` 函数也只能在 `Composeble` 函数中运行。这看起来似乎跟协程比较像，`@Composeble`是不是也像协程一样，往函数中添加了一些参数呢？          
