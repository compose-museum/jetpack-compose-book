...
接下来我们实现一个简单的纵向布局，里面的子元素从上到下依次排列，同时获取所有子元素的价格并进行累加，最后打印出来。
...
这里用到了绘制时的部分内容，此处不再深究。有兴趣的同学可以自行了解。
...
- 所有子微件都有 weight，按比例实现高度分配
- 父布局的宽高是确定的

所以代码的逻辑就是：读取所有 `weight` ，按比例分配高度就行。

首先类似于 `Column` ，我们也写一个 `VerticalScope` ，让我们自定义的 weight 只能在自定义的布局中使用
...
具体实现类似于之前的 `CountChildrenNumber`，不同之处在于我们要获取到各个 `WeightParentData` 的值并保存下来，计算总的 weight。这样就可以按比例分配高度了。
